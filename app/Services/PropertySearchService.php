      ...
      return  Property::with('propertyTranslation','propertyDetails','country.countryTranslation','state.stateTranslation','city.cityTranslation','category.categoryTranslation')
        ...
        // Bedroom
         ->when($data['bed'], function ($query) use ($data) {
            $query->whereHas('propertyDetails',function($query) use($data){
                $query->where('bed', $data['bed']);
            });
        })
        // min Price
        ->when($data['minPrice'], function ($query) use ($data) {
                $query->where('price','>=' ,(int)$data['minPrice']);
            
        })
        // max Price
        ->when($data['maxPrice'], function ($query) use ($data) {
            $query->where('price','<=' ,(int)$data['maxPrice']);

        })
        //bathroom
        ->when($data['bath'], function ($query) use ($data) {
            $query->whereHas('propertyDetails',function($query) use($data){
                $query->where('bath', $data['bath']);
            });
        })
        // type
        ->when($data['property_type'], function ($query) use ($data) {
            $query->where('type' ,'LIKE','%'.$data['property_type'].'%');
        ...
        // // elseif($data['category']=="" && $data['city']=="" && $data['minPrice']=="" && $data['maxPrice']=="" && $data['minArea']=="" && $data['maxArea']=="" && $data['bed']=="" && $data['bath']=="" && $data['title']=="" && $data['property_type'] && $data['area'] ){
        // //     // dd($data);
        // //     return $this->_propertySearchRepository->getByPropertyTypeCityArea($data);
        // // }
        // elseif($data['category']=="" && $data['city']=="" && $data['minPrice']=="" && $data['maxPrice']=="" && $data['minArea']=="" && $data['maxArea']=="" && $data['bed']=="" && $data['bath']=="" && $data['title']=="" && $data['property_type'] ){
        //     // dd('CategoryCityPriceAreaBedBath');
        //     return $this->_propertySearchRepository->getByPropertyType($data);
        // }
