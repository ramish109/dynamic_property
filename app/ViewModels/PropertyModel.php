...
class PropertyModel implements IPropertyModel
{
    ...
    public function add(Request $request)
      {
        ...
        if($request->package_id != null && $request->images){

            $package_enroll = PackageUser::where('id','=',$request->package_id)->get();
            $package_type = $package_enroll[0]->package_id;
            $Package = Package::where('id','=',$package_type)->get();
            $package_gallery_limit = $Package[0]->gallery_images_limit;
            $gallery_upload = sizeof($request->images);            
        
            if(!($gallery_upload <= $package_gallery_limit)){
                $gallery_upload_limit = 'Gallery upload limit in ('.$Package[0]->name.' Package) max '.$package_gallery_limit.' images';
            } else {
                $request['gallery_limit']='Gallery';
            }
        }
        ...
        request()->validate([
            ...
            'gallery_limit.required'=> $gallery_upload_limit ?? 'No gallery',
            ...
            'images=>.required'=> 'Gallery Image is required',
    ...
    public function update(Request $request, $id)
    {
        ...
        if($user->type !== 'admin'){
            ...
            $dataProperty['moderation_status'] = 1;
